...
RectangleBoundingObject::RectangleBoundingObject(float x, float y, float width, float height)
        : x{x}, y{y}, width{width}, height{height}, mesh(RectangleBoundingObject::rectangleVertices,
...
void RectangleBoundingObject::draw() {
    ...
    mesh.draw(ResourceManager::getShader("bounding_object"), modelMat);
}
...
CircleBoundingObject::CircleBoundingObject(float x, float y, float radius)
        : x{x}, y{y}, radius{radius}, mesh(CircleBoundingObject::circleVertices,
